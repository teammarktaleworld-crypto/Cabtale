...
class LocationController extends GetxController implements GetxService {
  ...
  void setPickUp(Address? address) {
    ...
    fromAddress = _hasValidCoordinates(address) ? address : null;
  }

  void setDestination(Address? address) {
    destinationLocationController.text = address?.address ?? '';
    toAddress = _hasValidCoordinates(address) ? address : null;
  }
  ...
  Future<List<PredictionModel>> searchLocation(BuildContext context, String text, {LocationType type = LocationType.from, bool fromMap = false}) async {
    ...
    if(!fromMap) {
      ...
      _clearSelectedAddressIfEdited(type, query);
      update();
    }
    ...
    if(query.isNotEmpty) {
      ...
      Response response = await locationServiceInterface.searchLocation(query);
      ...
      if (response.statusCode == 200 &&
          ...
          response.body['data']['predictions'] is List) {
        for (final dynamic prediction in response.body['data']['predictions']) {
          if (prediction is Map) {
            _predictionList.add(PredictionModel.fromJson(Map<String, dynamic>.from(prediction)));
          }
        }
      }

      if(!fromMap) {
        resultShow = _predictionList.isNotEmpty;
      }
      update();
    }else{
      _searchRequestId++;
  ...
  Future<Address?> setLocation(String placeID, String address, GoogleMapController? mapController, {LocationType type = LocationType.from, bool fromSearch = false}) async {
    ...
    selecting = true;
    update();
    LatLng? latLng = _latLngFromPlaceId(placeID);
    Address? selectedAddress;
    Response? response;
    if(latLng == null) {
      response = await locationServiceInterface.getPlaceDetails(placeID);
      if(response != null && response.statusCode == 200 && response.body['data']['status'] == 'OK') {
  ...
  bool _hasValidCoordinates(Address? address) {
    return address?.latitude != null && address?.longitude != null;
  }

  void _clearSelectedAddressIfEdited(LocationType type, String text) {
    final Address? selectedAddress = _getSelectedAddress(type);
    final String selectedText = selectedAddress?.address?.trim() ?? '';
  ...
  Address? _getSelectedAddress(LocationType type) {
    switch(type) {
      case LocationType.from:
        return fromAddress;
      case LocationType.to:
        return toAddress;
      case LocationType.extraOne:
        return extraRouteAddress;
      case LocationType.extraTwo:
        return extraRouteTwoAddress;
      case LocationType.senderLocation:
        return parcelSenderAddress;
      case LocationType.receiverLocation:
        return parcelReceiverAddress;
      default:
        return null;
    }
  }

  LatLng? _latLngFromPlaceId(String placeID) {
    if (!placeID.startsWith('coords:')) {
      return null;
    }

    final List<String> values = placeID.substring(7).split(',');
    if (values.length != 2) {
      return null;
    }

    final double? latitude = double.tryParse(values[0]);
    final double? longitude = double.tryParse(values[1]);
