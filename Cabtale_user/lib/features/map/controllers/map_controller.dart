...
class MapController extends GetxController implements GetxService {
  ...
  Future<void> searchDeliveryMen() async {
    ...
    Uint8List rideSafeCarIcon = carMarkerIcon;
    try {
      rideSafeCarIcon = await convertAssetToUnit8List(
          'assets/image/ridesafe_top.png',
          width: 50);
    } catch (_) {}
    ...
    for (int i = 0;
        ...
        i++) {
      final driver = Get.find<RideController>().nearestDriverList[i];

      Uint8List markerIcon;

      // 🧩 Choose which icon to show
      if (driver.category == 'motor_bike') {
        markerIcon = bikeMarkerIcon;
      } else if (driver.category == 'ridesafe_top') {
        markerIcon = rideSafeCarIcon; // 👈 your custom logo
      } else {
  ...
  void setFromToMarker(LatLng from, LatLng to,
    ...
    Uint8List toMarker =
        await convertAssetToUnit8List(Images.targetLocationIcon, width: 100);
